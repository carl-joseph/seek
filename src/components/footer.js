...
export default function Footer({ whiteBg = false }) {
    ...
    return (
        <footer className={`h-100vh flex pos-rel z-2 flex-col text-lg ${whiteBg ? "bg-light text-black" : "bg-black"}`}>
                ...
                <div className='text-center flex flex-col gap-10'>
                    ...
                    {cta?.email && (
                        <div className='fade--in' data-sal>
                            <a href={`mailto:${cta.email}`} className='f-20 underline op-50 op-link inverse mt10 block'>
                                {cta.email}
                            </a>
                        </div>
                    )}
